...
// Monitors for a target process and injects rendertest.dll at the earliest
// possible moment, replicating RenderDoc's normal suspended-process injection.
//
// Usage: process_injector.exe <dll_path> [process_name]
//   dll_path:     full path to rendertest.dll
...
{
    printf("=== RenderTest Process Injector ===\n\n");

    if(argc < 2)
    {
        printf("Usage: process_injector.exe <rendertest.dll path> [process_name]\n");
        printf("Example: process_injector.exe F:\\rdtest\\rendertest.dll\n");
        return 1;
    }
    ...
    if(ok)
    {
        printf("\n[+] SUCCESS! rendertest.dll injected.\n");
        printf("[*] Open qrendertest.exe to attach and capture frames (F12).\n");
    }
    else
    {
        printf("\n[!] FAILED to inject.\n");
